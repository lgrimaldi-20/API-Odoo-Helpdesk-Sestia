...
**Helpdesk export (read-only, `GET`):** `/helpdesk/export/tickets.csv` · `/historial.jsonl` · `/adjuntos.zip` · `/catalogos.zip` · `/catalogos` (JSON). Common query params: `tenant`, `limite` (cap for the 10-20 ticket sample). Extra: `?etapas_cierre=Resuelto,Cerrado` on `tickets.csv`; `?incluir_tracking=true` on `historial.jsonl`.
