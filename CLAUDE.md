...
- **[core/helpdesk_export.py](core/helpdesk_export.py)** — **read-only** export of Odoo Helpdesk tickets for the migration to the SESTIA Helpdesk module (spec: `migracion-tickets-odoo-helpdesk.pdf`). Unlike the rest of the middleware it does **not** write to Odoo and does **not** touch the state store (export is idempotent by nature — the destination's `odoo_ref` dedupes on re-import). Produces the three migration files plus the pre-load catalogs: `exportar_tickets_csv()` (one `helpdesk.ticket` per row, RFC 4180), `exportar_historial_jsonl()` (one `mail.message` of the chatter per line — classified `comentario`/`nota_interna`/`tracking`; tracking excluded by default), `exportar_adjuntos_zip()` (`ir.attachment` binaries + `manifiesto_adjuntos.csv`), `exportar_catalogos()`/`catalogos_a_zip()` (teams, stages, categories, tags, users), and `contar_volumenes()` (pre-export counts — step 2 of the plan). **Key mapping decisions:** the `open`/`closed` status is derived from the stage's `fold` field (folded kanban stages = closing stages), overridable by an explicit name list (`HELPDESK_ETAPAS_CIERRE` env var or `?etapas_cierre=` query param); dates are emitted as ISO 8601 with an explicit `Z` (Odoo returns naive UTC); message bodies (HTML in Odoo) are delivered as plain text. Like `inventario.py`, it has a fixed schema and does **not** use `mappings.yaml`. Raises `HelpdeskExportError`.

  **Full-spec details** (all sections of the migration doc are covered):
  - **`odoo_ref` is the join key across all three files** — resolved ONCE per export by `_mapa_refs()` (prefers the visible ticket number, falls back to the id) and reused by tickets, historial and the attachment manifest, including the ZIP folder names. Never re-derive it locally: an earlier version had the manifest use the raw `res_id` while the other two used `ticket_ref`, which orphaned **every** attachment on import. `_seguro_para_ruta()` sanitizes it for ZIP paths (a ref like `SOP/2024/001` would otherwise create nested folders), while the manifest keeps the unsanitized value.
  - **Catalogs must cover what the tickets reference.** `_catalogo_usuarios()` includes users referenced as assignee/creator **even if archived** (`active_test: False` + explicit id list) and flags them with `activo: false` — the spec's criterion is "appears as assignee or author", not "is currently active". `_catalogo_etapas()` takes the same `etapas_cierre` override as the tickets export so `es_cierre` can't contradict the exported status. Subcategories are their own catalog, separate from categories.
  - **`validar_contra_catalogos()`** (`GET /validar`) cross-checks every ticket's team, stage, category, subcategory, tags and emails against the catalogs before importing, plus closed-tickets-without-`fecha_cierre`. Tags matter most: their catalog is **closed** (not created on the fly).
  - **Fail loudly, never degrade silently.** If `mail.message.subtype` can't be read the export **raises** instead of defaulting to empty — that default would classify every internal note as a public comment, exposing agents' private notes to customers. Tags whose name can't be resolved are logged rather than silently dropped from the row.
  - **Cut-off window (step 8).** Every export function takes `desde`/`hasta`, filtering on the ticket's **`write_date`** (not `create_date`) so the incremental re-export also picks up tickets *modified* after the cut-off. `_dominio_tickets()` builds the domain; `_a_fecha_odoo()` strips the ISO offset because Odoo compares against naive UTC. `_ids_tickets()` and `_leer_tickets()` share the same domain and `order="id"`, so the three files always cover **the same set of tickets**.
  - **Custom fields (§3).** `_campos_personalizados()` auto-detects the client's `x_*` fields via `fields_get` and appends one column per field **at the end** of the CSV header. Relational/binary types are skipped — they don't fit in a cell.
  - **Embedded images (§5.3).** Bodies reference images as `/web/image/N` or `data:` URIs; both die when Odoo is shut down. `imagenes_embebidas()` extracts them and `_adjuntos_embebidos()` promotes them into the ZIP — including attachments whose `res_model` is `mail.message` (so a plain per-ticket attachment search misses them). Inline base64 images get a synthetic **negative** id (they are not a real `ir.attachment`) and are materialized as `inline_<msg>_<n>.<ext>`. The message keeps an `imagenes_embebidas` field listing the ids so the importer can relink them.
  - **Quoted threads (§5.3).** `html_a_texto(..., recortar_citas=True)` (the default in the historial export) drops `<blockquote>` / `gmail_quote` / `o_mail_quote` / signature containers, leaving only the new body.
  - **Author email.** Resolved from `res.partner.email` via `_emails_de_partners()`; `email_from` is only a fallback and is cleaned with `_email_limpio()` since it's a mail header (`"Nombre <mail@x>"`), not an address.
  - **Memory.** Attachment binaries are **never** all loaded at once: metadata is read first, then `_LectorBinarios` pulls `datas` in blocks of `LOTE_ADJUNTOS` (50), discarding the previous block, so the memory peak is independent of the migration size. `exportar_adjuntos_zip(destino=<file>)` streams to disk instead of memory.
  - **History scope (§7).** `solo_abiertos=True` restricts the chatter to open tickets via `_filtrar_abiertos()`.
  - **`fecha_cierre` (§3).** Required for closed tickets: if a ticket is in a closing stage with no `close_date`, the export **logs a warning** listing the refs rather than silently emitting a blank the importer will reject.
...
### Routers (`routers/`)
`/facturas`, `/pagos`, `/conciliar`, `/stock/*`, `/helpdesk/export/*` — each protected by the shared **[core/seguridad.py](core/seguridad.py)** (`verify_api_key` reads `API_KEY` from env each call; `resolver_tenant` → 400 on unknown tenant). Extracted to a neutral module so routers don't import `api.py` (avoids a circular import). `/facturas`, `/pagos` and `/stock/ajustar` take an optional `"async": true` field → enqueue and return `{encolado, task_id}` (in eager mode this still runs inline); otherwise synchronous. `api.py` also has `GET /estado/{entidad}/{id_origen}` to read a record's sync state.

### Endpoints
`/odoo` (generic, auth) · `/facturas` · `/pagos` · `/conciliar` · `/stock/ajustar` · `/stock/consultar` · `/estado/{entidad}/{id_origen}` · `/health` (no auth).

**Helpdesk export (read-only, `GET`):** `/helpdesk/export/volumenes` (JSON counts) · `/tickets.csv` · `/historial.jsonl` · `/adjuntos.zip` · `/catalogos.zip` · `/catalogos` (JSON).

Common query params: `tenant`, `limite` (cap for the 10-20 ticket sample), and `desde`/`hasta` (ISO 8601 cut-off window on `write_date` — the incremental re-export of step 8; `odoo_ref` dedupes on the destination side). Extra: `?etapas_cierre=Resuelto,Cerrado` on `tickets.csv`; `?incluir_tracking=true`, `?recortar_citas=false`, `?solo_abiertos=true` on `historial.jsonl`; `?incluir_embebidas=false` on `adjuntos.zip`.

**Migration order:** `/volumenes` first (agree on scope and whether the ZIP needs splitting), then `/catalogos` (pre-load in SESTIA), then the sample with `?limite=20`, then the full export. Split a large ZIP with the date window or `limite`.
